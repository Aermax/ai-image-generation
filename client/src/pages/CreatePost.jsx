...
const CreatePost = () => {
  ...
  const generateImage = async () => {
    if (form.prompt) {
      try {
        ...
        const response = await fetch('https://aermax-ai.onrender.com/api/v1/dalle', {
          method: "POST",
          headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    if (form.prompt && form.photo) {
      setLoading(true);
      try {
        const response = await fetch('https://aermax-ai.onrender.com/api/v1/post', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
          ...
          <div className='mt-5 flex gap-5'>
            <button
              ...
              className=" text-white bg-green-700 font-medium rounded-md text-sm
                   w-full  px-5 py-2.5 sm:w-auto text-center"
            >
              {genratingImg ? "Generating..." : "Generate Image"}

            </button>
          </div>
          <div className='mt-10'>
            <p className="mt-2 text-[#666e75] text-[14px]  ">
              Once you created image you can
              share with others in the community
            </p>
            <button
              type="button"
              onClick={handleSubmit}
              className="mt-3 text-white bg-[#6369ff] font-medium rounded-md text-sm
